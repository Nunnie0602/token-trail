...
- **API P95 響應延遲 (SLA Compliance) < 50ms：** 快取命中時，端到端的 HTTP 響應時間。
...
| **Backend Core** | FastAPI (Python 3.11) | 利用 `asyncio` 機制實現高效能非同步非阻塞事件處理，負責高吞吐的 `/step` 路由與背景預載 Task 調度。 |
...
| **Observability Design** | Python Structlog | **Logs 觀測：** 輸出結構化 JSON 日誌（包含 `session_id` 與 `trace_id`），在非同步環境下精準追蹤每一次 Step 的快取命中或 Fallback 行為。 |
...
- **Phase 2：Production Pipe & Backend (核心目標：Production Engineering)**
    - **交付產出：** 搭建 FastAPI 與 Redis 基礎架構。**同步建立 GitHub Actions 自動化管線**（整合 Pytest、Ruff、MyPy）。實作多路投機預載快取（Multi-path Speculative Prefetch），進行 P95 延遲壓測，確保後端交付品質。
...
- **效能與 SLA：** 在快取命中情況下，Redis 讀取開銷  $< 5\text{ms}$，包含網路來回的 API P95 響應延遲必須 $< 50\text{ms}$。前端 Canvas 穩定維持在 $60\text{ FPS} \pm 2\text{ FPS}$。
